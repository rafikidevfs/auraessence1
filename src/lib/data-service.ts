...
export async function loadCatalogSnapshot(): Promise<CatalogSnapshot> {
  ...
  // 1. Tenta carregar primeiro do localStorage para respeitar imediatamente exclusões locais
  try {
  ...
  // 2. Se não tem no localStorage, tenta buscar do Supabase
...
export async function deleteCategory(slug: string): Promise<void> {
  // 1. Grava permanentemente na lista de excluídos do localStorage
  if (typeof window !== "undefined") {
    const existingDeleted = getDeletedCategories();
  ...
  // 3. Atualiza o snapshot atual e remove a categoria da marra do cache e do localStorage principal
